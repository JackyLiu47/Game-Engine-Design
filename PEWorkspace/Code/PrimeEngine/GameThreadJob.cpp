...
int ClientGame::runGameFrame()
{
	...
	{
		Handle hphysEndEvent("EVENT", sizeof(Event_PHYSICS_END));
		Event_PHYSICS_END* physEndEvent = new(hphysEndEvent) Event_PHYSICS_END;

		physEndEvent->m_frameTime = m_frameTime;
		Events::EventQueueManager::Instance()->add(hphysEndEvent, Events::QT_GENERAL);
	}
