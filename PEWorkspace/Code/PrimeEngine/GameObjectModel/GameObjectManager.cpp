	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				/*if (pRealEvent->m_enableFullPhysics) {
					pPhysicsComponent->enableGravity();
					pPhysicsComponent->m_collider = PhysicsComponent::SPHERE;
					pPhysicsComponent->setVelocity(Vector3(5, 10, 0));
					pPhysicsComponent->setAVelocity(Vector3(0, 3, 0));
					pPhysicsComponent->m_bounce = 0.5;
					pPhysicsComponent->m_angularAttenuation = 0.2;
				}*/
				pSN->m_hPhysics = hPhysicsComponent;
			}
			else
			{
